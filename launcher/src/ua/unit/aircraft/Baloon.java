    ...
    public void    updateConditions(){
        ...
        switch (weather){
            case "SUN": {
                ...
                Simulator.printWriter.println("Helicopter#" + this.name + "(" + this.id + "): Mmmmmmm... You're so hot, Sunny");
                break;
            }
            case "RAIN": {
                if (height <= 5) {
                    ...
                    coordinates = new Coordinates(coordinates.getLongitude() + 5, coordinates.getLatitude(),
                            height - 5);
                    Simulator.printWriter.println("Helicopter#" + this.name + "(" + this.id + "): I feel those little drops everywhere on my body. I'm moisty");
                }
                break;
            }
            case "FOG": {
                if (height <= 3) {
                    ...
                    coordinates = new Coordinates(coordinates.getLongitude(), coordinates.getLatitude(),
                            height - 3);
                    Simulator.printWriter.println("Helicopter#" + this.name + "(" + this.id + "): I can't see you. But I feel you behind the Fog.");
                }
                break;
            }
            case "SNOW": {
                if (height <= 15) {
                    ...
                    Simulator.printWriter.println("Helicopter#" + this.name + "(" + this.id + "): Every time I leak snow is like the first time.");
                    coordinates = new Coordinates(coordinates.getLongitude(), coordinates.getLatitude(),
                            height - 15);
                }
                break;
